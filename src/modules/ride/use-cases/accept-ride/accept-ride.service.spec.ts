...
import { AccountFactory, AccountPropsCreation } from 'src/modules/account/domain/factories/account.factory';
import { IAccountRepository } from 'src/modules/account/persistence/repositories/account-repository.interface';
import { Ride } from '../../domain/entities/ride.entity';
import { RideFactory, RidePropsCreation } from '../../domain/factories/ride.factory';
...
const accountProps = createMock<AccountPropsCreation>({
...
describe('AcceptRideUseCase', () => {
  ...
  it('should throw an error when ride status is not requested', async() => {
    ...
    const rideProps = createMock<RidePropsCreation>({ status: 'accepted' });
  ...
  it('should throw an error if driver has another accepted or in progress ride', async() => {
    ...
    const ride = createMock<Ride>({ cannotBeAccepted: () => false });
    mockRideRepository.findById.mockResolvedValue(ride);
